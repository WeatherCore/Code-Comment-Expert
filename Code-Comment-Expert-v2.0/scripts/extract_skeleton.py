...
def _parse_python(source: str, clean_lines: list[str]) -> tuple[list[dict], list[str], str, list[str]]:
    classes: list[dict] = []
    imports: list[str] = []
    for m in _IMPORT_RE_PY.finditer(source):
        imports.append(m.group(1) or m.group(2))
    referenced = [imp.split(".")[0] for imp in imports]

    for cm in _CLASS_RE_PY.finditer(source):
        name = cm.group("name")
        start = source[: cm.start()].count("\n")
        body_start_line = start + 1
        # 类体 = 缩进大于类声明行的所有行；用 clean 行避免 docstring 干扰
        cls_indent = len(clean_lines[start]) - len(clean_lines[start].lstrip())
        methods: list[dict] = []
        end_line = body_start_line
        for j in range(body_start_line, len(clean_lines)):
            ln = clean_lines[j]
            if not ln.strip():
                continue
            ...
            if cur_indent <= cls_indent:
                break
            mm = _METHOD_RE_PY.match(ln)
            if mm:
                methods.append({
                    "name": mm.group("name"),
                    "signature": ln.strip()[:200],
                    "annotations": [d.strip().split("(")[0] for d in (mm.group("deco") or "").split() if d.startswith("@")][:5],
                    "line": j + 1,
                })
            end_line = j + 1
        classes.append({
            "name": name,
            "type": "class",
            "bases": [b.strip() for b in (cm.group("bases") or "").split(",") if b.strip()],
            "annotations": [],
            "methods": methods,
            "line": start + 1,
            "end_line": end_line,
        })
    return classes, imports, "", referenced
...
def extract_file(p: Path, root: Path, project_type_names: set[str] | None = None) -> dict | None:
    ...
    if lang in ("java", "kotlin", "csharp", "php", "c", "cpp"):
        ...
        classes, imports, package, referenced = _parse_python(clean, clean_lines)
        # 顶层函数：行首无缩进的模块级 def
        funcs = []
        for fm in _METHOD_RE_PY.finditer(clean):
            ln = _line_of(clean, fm.start())
            if not clean_lines[ln - 1].startswith((" ", "\t")):
                funcs.append({"name": fm.group("name"), "signature": fm.group(0).replace("\n", " ").strip()[:200], "line": ln})
        package = ""
